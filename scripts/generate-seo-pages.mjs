...
const pages = [
  ...
  ...locationModules.map(([slug, module, title, description, body, area]) => page(slug, title, description, `${body} SV CurioTech supports ${module} learners in ${area.split(",")[0]} with practical examples, flexible batches, SAP training guidance, interview preparation, resume support and placement assistance.`, { h1: title.split("|")[0].trim(), schemas: [courseSchema(title.split("|")[0].trim(), description, `${siteUrl}/${slug}`, area), faqSchema()] })),
